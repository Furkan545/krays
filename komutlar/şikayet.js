...
exports.run = function(client, message, args) {
    message.delete();
    if(args[0] == "yardım"){
      message.reply("Kullanım: k+report <kullanıcı> <Sebep>");
      return;
    }
    ...
    let reportschannel = message.guild.channels.find(`name`, "°•kullanıcı-şikayetler•°");
    if(!reportschannel) return message.channel.send(" `°•kullanıcı-şikayetler•°` İsminde Yazı Kanalı Bulamıyorum.!");
